...
#include "./BSP/ATK_MD0240/atk_md0240_font.h"
...
void atk_md0240_init(void)
{
    ...
    atk_md0240_clear(ATK_MD0240_CYAN);
	atk_md0240_ShowChinese(10, 50, "网络连接中，请稍候", ATK_MD0240_BLACK, ATK_MD0240_CYAN, 24, 0);
...
/**
 * @brief       ATK-MD0240模块LCD填充矩形
 * @param       x1   : 待填充矩形的左上角X坐标
 *              y1   : 待填充矩形的左上角Y坐标
 *              x2   : 待填充矩形的右下角X坐标
 *              y2   : 待填充矩形的右下角Y坐标
 *              color: 待填充矩形的颜色
 * @retval      无
 */
void atk_md0240_fill_rect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    for (uint16_t y = y1; y <= y2; y++) {
        atk_md0240_draw_line(x1, y, x2, y, color); // 在每一行中绘制从x1到x2的线
    }
}
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void atk_md0240_ShowChinese(uint16_t x,uint16_t y,uint8_t *s,uint16_t fc,uint16_t bc,uint8_t sizey,uint8_t mode)
{
	while(*s!=0)
	{
		if(sizey==12) atk_md0240_Chinese12x12(x,y,s,fc,bc,sizey,mode);
		else if(sizey==16) atk_md0240_Chinese16x16(x,y,s,fc,bc,sizey,mode);
		else if(sizey==24) atk_md0240_Chinese24x24(x,y,s,fc,bc,sizey,mode);
		else if(sizey==32) atk_md0240_Chinese32x32(x,y,s,fc,bc,sizey,mode);
		else return;
		s+=2;
		x+=sizey;
	}
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void atk_md0240_Chinese12x12(uint16_t x,uint16_t y,uint8_t *s,uint16_t fc,uint16_t bc,uint8_t sizey,uint8_t mode)
{
	uint8_t i,j,m=0;
	uint16_t k;
	uint16_t HZnum;//汉字数目
	...
	for(k=0;k<HZnum;k++) 
	{
		...
		{ 	
			atk_md0240_set_address(x,y,x+sizey-1,y+sizey-1);
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					if(!mode)//非叠加方式
					{
						if(tfont12[k].Msk[i]&(0x01<<j))atk_md0240_write_dat_16b(fc);
						else atk_md0240_write_dat_16b(bc);
						m++;
						if(m%sizey==0)
						{
							m=0;
					...
					{
						if(tfont12[k].Msk[i]&(0x01<<j))	atk_md0240_draw_point(x,y,fc);//画一个点
						x++;
						if((x-x0)==sizey)
						{
							x=x0;
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void atk_md0240_Chinese16x16(uint16_t x,uint16_t y,uint8_t *s,uint16_t fc,uint16_t bc,uint8_t sizey,uint8_t mode)
{
	uint8_t i,j,m=0;
	uint16_t k;
	uint16_t HZnum;//汉字数目
		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					if(!mode)//非叠加方式
					{
						if(tfont16[k].Msk[i]&(0x01<<j))atk_md0240_write_dat_16b(fc);
						else atk_md0240_write_dat_16b(bc);
						m++;
						if(m%sizey==0)
						{
					...
					{
						if(tfont16[k].Msk[i]&(0x01<<j))	atk_md0240_draw_point(x,y,fc);//画一个点
						x++;
						if((x-x0)==sizey)
						{
							x=x0;
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void atk_md0240_Chinese24x24(uint16_t x,uint16_t y,uint8_t *s,uint16_t fc,uint16_t bc,uint8_t sizey,uint8_t mode)
{
	uint8_t i,j,m=0;
	uint16_t k;
	uint16_t HZnum;//汉字数目
		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					if(!mode)//非叠加方式
					{
						if(tfont24[k].Msk[i]&(0x01<<j))atk_md0240_write_dat_16b(fc);
						else atk_md0240_write_dat_16b(bc);
						m++;
						if(m%sizey==0)
						{
					...
					{
						if(tfont24[k].Msk[i]&(0x01<<j))	atk_md0240_draw_point(x,y,fc);//画一个点
						x++;
						if((x-x0)==sizey)
						{
							x=x0;
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void atk_md0240_Chinese32x32(uint16_t x,uint16_t y,uint8_t *s,uint16_t fc,uint16_t bc,uint8_t sizey,uint8_t mode)
		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					if(!mode)//非叠加方式
					{
						if(tfont32[k].Msk[i]&(0x01<<j))atk_md0240_write_dat_16b(fc);
						else atk_md0240_write_dat_16b(bc);
						m++;
						if(m%sizey==0)
						{
					...
					{
						if(tfont32[k].Msk[i]&(0x01<<j))	atk_md0240_draw_point(x,y,fc);//画一个点
						x++;
						if((x-x0)==sizey)
						{
							x=x0;
