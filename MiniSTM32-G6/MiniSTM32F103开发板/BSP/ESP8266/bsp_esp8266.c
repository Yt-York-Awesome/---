...
//// 解析和处理 MQTT 消息的函数
//void parseMqttMessage(const char* message) 
//{
//    int clientId, qos;
...
//    
//    // 使用 sscanf 解析消息字符串
//    if (sscanf(message, "+MQTTSUBRECV:%d,\"%[^\"]\",%d,%[^\n]", &clientId, topic, &qos, payload) >= 3)
//	{
//        // 输出解析结果
//        printf("Client ID: %d\r\n", clientId);
//        printf("Topic: %s\r\n", topic);
...
//        // 在这里可以根据 payload 进一步处理，如 JSON 解析
//        // 可以调用一个处理函数来解析 payload 的 JSON 内容
//        // 如：processJsonPayload(payload);
//    }
//	else
//	{
//        printf("Failed to parse the message: %s\n", message);
//    }
//}
